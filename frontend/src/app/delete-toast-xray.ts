    ...
    <div class="row" [style.display]="state().value === 'inactive' ? 'none' : ''">
      ...
      <div class="col-9">
        @if (!options.closeButton) {
          <a class="deleteButton btn btn-pink btn-sm" (click)="deleteToast($event)">
            {{ deleteString }}
          </a>
        }
        @if (!options.closeButton) {
          <a class="deleteButton btn btn-pink btn-sm" (click)="xrayEnabled = true; deleteToast($event)">
            Delete with xRay
          </a>
        }
        @if (!options.closeButton) {
...
export class XrayToast extends Toast implements OnInit {
	...
	deleteToast(event: Event){
		...
		switch (this.nameComponent){
			case 'scenario': this.scenarioService.deleteScenarioEmitter(this.xrayEnabled);
				break;
