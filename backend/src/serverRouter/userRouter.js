...
import * as userMng from "../../dist/helpers/userManagement.js";
import projectMng from "../../dist/helpers/projectManagement.js";
import * as issueTracker from "../../dist/models/IssueTracker.js";
...
router.get('/stories', async (req, res) => {
    ...
    if (source === 'github' || !source)
        try {
            if (!userMng.checkValidGithub(req.query.githubName, req.query.repository))
                console.log('Username or Reponame not valid');
            ...
            if (response.status === 401)
                res.status(401).json('Github Status 401');
        ...
        try {
            await fetch(`http://${Host}/rest/api/2/search?jql=project=${projectKey}+AND+labels=Seed-Test&startAt=0&maxResults=200`, options)
...
router.get('/callback', (req, res) => {
    ...
    fetch(TOKEN_URL, {
        ...
        .then((data) => {
        if (data.error)
            throw Error('github user register failed');
        else
            userMng.getGithubData(res, req, data.access_token);
    })
        .catch((error) => {
