...
router
  ...
  .use((req, res, next) => {
    ...
    res.header(
      ...
      "Origin, X-Requested-With, Content-Type, Accept, Credentials, Authorization, X-Redirect"
    );
    next();
  })
  .use((_, __, next) => {
...
router.post("/", async (req, res) => {
  try {
    const db_id = await mongo.createStory(
      req.body.title,
...
router.delete("/:repo_id/:_id", async (req, res) => {
  try {
	...
	// ???????
    await helper.deleteFeatureFile(story.value.title, story.value._id);
...
router.get("/:story_id/:source/:_id", async (req, res) => {
  try {
    const scenario = await mongo.getOneScenario(
      req.params.story_id,
      req.params.source,
...
router.post("/:story_id/:source", async (req, res) => {
  try {
    const scenario = await mongo.createScenario(
      req.params.story_id,
      req.params.source,
      req.body.name
    );
...
router.put("/:story_id/:source/:_id", async (req, res) => {
  ...
  try {
    ...
    const updatedStory = await mongo.updateScenario(
      req.params.story_id,
      req.params.source,
      scenario
    );
...
router.get("/download/story/:source/:_id", async (req, res) => {
  try {
    ...
    const file = await helper.exportSingleFeatureFile(
      req.params._id,
      req.params.source
    );
    console.log(file);
