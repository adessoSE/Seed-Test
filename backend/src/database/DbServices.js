import * as mongodb$0 from "mongodb";
...
const { ObjectId } = mongodb$0;
const mongodb = mongodb$0;
...
dbConnection
    .establishConnection()
    .then(() => console.log("\x1b[32m%s\x1b[0m \x1b[33m%s\x1b[0m", "Connected to database @", dbConnection.getConnection().client.s.options.srvHost));
/**
...
async function updateGithubToken(objId, updatedToken) {
    try {
        ...
        return await db
            .collection(userCollection)
            .updateOne({ _id: new ObjectId(objId) }, { $set: { "github.githubToken": updatedToken } });
    }
    catch (e) {
...
async function mergeGithub(userId, login, id) {
    try {
        ...
        if (githubAccount.hasOwnProperty("jira") &&
...
async function updateStory(updatedStory, session = undefined, client = undefined) {
    try {
        const db = session
        ...
        return await collection.findOneAndReplace({ _id: new ObjectId(updatedStory._id.toString()) }, updatedStory, { returnDocument: "after", session: session || undefined });
    }
    catch (e) {
...
async function createStoryGroup(repoID, name, members, sequence, session = undefined, client = undefined) {
    try {
        const db = session
            ? client.db('Seed', session)
            : dbConnection.getConnection();
        const groups = await db.collection(repositoriesCollection).findOneAndUpdate({ _id: new ObjectId(repoID) }, {
            $push: {
                groups: {
                    _id: new ObjectId(),
                    name,
...
async function updateStoryGroup(repoId, groupId, updatedGroup, session = undefined, client = undefined) {
    try {
        const db = session
            ? client.db('Seed', session)
            : dbConnection.getConnection();
...
async function updateStoryGroupsArray(repoId, groupsArray) {
    try {
        ...
        return await db
            .collection(repositoriesCollection)
            .findOneAndUpdate({ _id: new ObjectId(repoId) }, { $set: { groups: groupsArray } }, { projection: { groups: 1 } });
    }
    catch (e) {
...
async function showSteptypes() {
    try {
        const db = dbConnection.getConnection();
        return await db.collection(stepTypesCollection).find({}).toArray();
    }
    catch (e) {
...
async function createStory(storyTitle, storyDescription, repoId, session = undefined, client = undefined) {
    ...
    try {
        ...
        if (repo && repo.stories.length > 0) {
            ...
            finalIssueNumber =
                iNumberArray.findIndex((_, i) => !iNumberArray.includes(i + 1)) + 1;
            if (finalIssueNumber === 0)
                finalIssueNumber = iNumberArray.length + 1;
        }
...
async function deleteStory(repoId, storyId) {
    ...
    try {
        ...
        try {
            ...
            for (const index in groups.groups)
...
async function insertStoryIdIntoRepo(storyId, repoId, session = undefined, client = undefined) {
    try {
        const db = session
            ? client.db('Seed', session)
            : dbConnection.getConnection();
        return await db
            .collection(repositoriesCollection)
            .findOneAndUpdate({ _id: new ObjectId(repoId) }, { $push: { stories: new ObjectId(storyId) } }, { session: session || undefined });
    }
    catch (e) {
...
async function updateScenarioList(storyId, scenarioList) {
    try {
        ...
        return await db
            .collection(storiesCollection)
            .findOneAndUpdate({ _id: new ObjectId(storyId) }, { $set: { scenarios: scenarioList } });
    }
    catch (e) {
        console.log(`ERROR in insertStoryIdIntoRepo: ${e}`);
        throw e;
...
async function getAllStoriesOfRepo(repoId) {
    ...
    try {
        ...
        const repo = await db
            ...
            .findOne({ _id: new ObjectId(repoId) });
        if (repo)
            for (const entry of repo.stories) {
                const story = await db
                    .collection(storiesCollection)
...
async function getOneScenario(storyId, scenarioId) {
    try {
        ...
        const scenarios = await db
            .collection(storiesCollection)
            .findOne({ _id: new ObjectId(storyId), "scenarios.scenario_id": scenarioId }, { projection: { scenarios: 1 } });
...
async function updateScenario(storyId, updatedScenario) {
    try {
        ...
        const collection = await db.collection(storiesCollection);
        return collection
            .findOneAndUpdate({ _id: new ObjectId(storyId) }, { $set: { "scenarios.$[it]": updatedScenario } }, {
...
async function deleteScenario(storyId, scenarioId) {
    try {
        ...
        const collection = await db.collection(storiesCollection);
        return collection
            .findOneAndUpdate({ _id: new ObjectId(storyId) }, { $pull: { scenarios: { scenario_id: scenarioId } } }, { returnDocument: "after" })
            .then((res) => res);
    }
    catch (e) {
...
async function updateRepository(repoID, newName, globalSettings) {
    try {
        ...
        const updatedRepo = await collection.findOneAndUpdate(repoFilter, { $set: updateFields }, { returnDocument: 'after' });
...
async function updateOwnerInRepo(repoId, newOwnerId, oldOwnerId) {
    try {
        ...
        await db
            ...
            .findOneAndUpdate({ Repo: new ObjectId(repoId) }, { $set: { owner: newOwner.email }, $push: { Members: wgMember } });
        return "Success";
    }
    catch (e) {
...
async function updateStoriesArrayInRepo(repoId, storiesArray) {
    ...
    try {
        ...
        return await db
            .collection(repositoriesCollection)
            .findOneAndUpdate({ _id: new ObjectId(repoId) }, { $set: { stories: sortedStoriesArray } }, { returnNewDocument: true });
    }
    catch (e) {
...
async function setIsSavedTestReport(testReportId, isSaved) {
    try {
        const db = dbConnection.getConnection();
        db.collection(ReportDataCollection).updateOne({ _id: new ObjectId(testReportId) }, {
            $set: { isSaved },
        });
    }
    catch (e) {
...
async function updateStoryStatus(storyId, storyLastTestStatus) {
    try {
        const db = dbConnection.getConnection();
        db.collection(storiesCollection).updateOne({ _id: new ObjectId(storyId) }, {
...
async function updateBlock(blockId, updatedBlock, session = undefined, client = undefined) {
    try {
...
async function addMember(repoId, user) {
    try {
        ...
        if (check)
            return "Dieser User ist bereits in der Workgroup";
        const repo = await db
            .collection(repositoriesCollection)
            .findOne({ _id: new ObjectId(repoId) });
        const owner = await db
...
async function updateMemberStatus(repoId, user) {
    try {
        ...
        const updatedWG = await wGCollection.findOneAndUpdate({ Repo: new ObjectId(repoId) }, { $set: { "Members.$[elem].canEdit": Boolean(user.canEdit) } }, { arrayFilters: [{ "elem.email": user.email }] });
        if (updatedWG) {
            const wG = await wGCollection.findOne({ Repo: new ObjectId(repoId) });
            const result = { owner: {}, member: [] };
            result.owner = { email: owner.email, canEdit: true };
...
async function getMembers(id) {
    try {
        ...
        const wG = await db
            ...
            .findOne({ Repo: new ObjectId(id) });
        if (!wG)
            return { owner: { email: owner.email, canEdit: true }, member: [] };
        const result = { owner: {}, member: [] };
        result.owner = { email: owner.email, canEdit: true };
        result.member = wG.Members;
        return result;
...
async function importStories(post, importRepo, session, storyFiles, groupMapping, existingNameList, importMode, checkAndAddSuffix, findAssociatedID, client, file, allConflicts = []) {
    ...
    function insertNewId(oldID, newID) {
        ...
        console.error(`Mapping with oldID: ${oldID} not found for newID insertion.`);
    }
    ...
    for (const storyFile of storyFiles) {
        ...
        if (post || importMode) {
            const newStory = await createStory(name, storyObject.body === null ? undefined : storyObject.body, importRepo, session, client);
            //Insert new ID corresponding to old one
            insertNewId(groupMapping[storyFiles.indexOf(storyFile)].oldID, newStory.toHexString());
            ...
            await insertStoryIdIntoRepo(newStory, importRepo, session, client);
        }
...
async function importBlocks(post, importRepo, repoName, session, existingNameList, repoBlocksData, importMode, checkAndAddSuffix, findAssociatedID, client, allConflicts = []) {
    for (const singularBlock of repoBlocksData) {
        if (!post && importMode)
            singularBlock.name = checkAndAddSuffix(singularBlock.name, existingNameList);
...
async function importGroups(post, importRepo, session, groupFiles, groupMapping, existingNameList, importMode, repo_id, checkAndAddSuffix, findAssociatedID, client, file, allConflicts = []) {
    const zip = new AdmZip(file.buffer);
    for (const groupFile of groupFiles) {
        ...
        if (!post && !importMode && findAssociatedID(name, allConflicts)) {
            ...
            let oldGroup = await getOneStoryGroup(repo_id, groupObject._id);
            groupObject.member_stories = oldGroup.member_stories;
            const updatedGroup = await updateStoryGroup(repo_id, groupObject._id, groupObject, session, client);
            console.log("Group " + name + " has been updated. ID: " + updatedGroup._id);
        }
        else {
            await createStoryGroup(importRepo, name, groupObject.member_stories, groupObject.isSequential, session, client);
            ...
            setTimeout(() => {
                fs.unlink(destinationPath, (err) => {
                    if (err)
                        ...
                        console.log(`${fileInfo[0].filename} deleted.`);
                });
            }, 18000000); // 5h Timeout
...
export { fileUpload };
export { deleteFile };
export { setIsSavedTestReport };
export { deleteReport };
export { getTestReports };
export { getGroupTestReports };
export { getReportByName };
export { getReportById };
export { getReportDataById };
export { uploadReport };
export { disconnectGithub };
export { mergeGithub };
export { findOrRegisterGithub };
export { getUserByGithub };
export { getUserById };
export { registerUser };
export { getUserByEmail };
export { showSteptypes };
export { deleteBackground };
export { updateBackground };
export { getOneScenario };
export { createScenario };
export { updateScenario };
export { deleteScenario };
export { updateScenarioList };
export { createStory };
export { deleteStory };
export { insertStoryIdIntoRepo };
export { getOneStory };
export { upsertEntry };
export { updateStory };
export { deleteUser };
export { updateUser };
export { getUserData };
export { createJiraRepo };
export { updateStoriesArrayInRepo };
export { getRepository };
export { deleteRepository };
export { getOneRepository };
export { getOneGitRepository };
export { getOneJiraRepository };
export { getAllStoriesOfRepo };
export { createRepo };
export { createStoryGroup };
export { updateStoryGroup };
export { deleteStoryGroup };
export { addToStoryGroup };
export { removeFromStoryGroup };
export { getAllStoryGroups };
export { getOneStoryGroup };
export { updateStoryGroupsArray };
export { createResetRequest };
export { getResetRequest };
export { deleteRequest };
export { getResetRequestByEmail };
export { saveBlock };
export { updateBlock };
export { getBlock };
export { getBlocks };
...
export { getMembers };
export { removeFromWorkgroup };
export { updateOneDriver };
export { updateScenarioStatus };
export { updateStoryStatus };
export { getAllSourceReposFromDb };
export { createGitRepo };
export { updateOwnerInRepo };
export { updateRepository };
export { getOneRepositoryById };
export { getRepoSettingsById };
export { importStories };
export { importBlocks };
export { importGroups };
export default {
    getFileList,
