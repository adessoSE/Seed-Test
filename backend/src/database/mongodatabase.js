...
async function getOneStory(storyId) {
    ...
    try {
        ...
        if (!story)
            story = await collection.findOne({ _id: new ObjectId(storyId) });
        return story;
    }
    catch (e) {
...
async function createStoryGroup(repoID, name, members, sequence) {
    ...
    try {
        ...
        const groups = await collection.findOneAndUpdate({ _id: new ObjectId(repoID) }, {
            $push: {
                groups: {
...
async function createGitOwnerRepoIfNoneExists(ownerId, githubId, gitOwnerId, repoName, source) {
    ...
    try {
        ...
        if (result === null) {
            ...
            if (repo.gitOwner === githubId)
                repo.owner = new ObjectId(ownerId);
            return repo;
        }
        return result._id;
    }
...
async function getTestReports(storyId) {
    ...
    try {
        ...
        result = await collection.find({ storyId: new ObjectId(storyId) }, { projection: { jsonReport: 0, reportOptions: 0, json: 0 } }).toArray();
...
async function getGroupTestReports(storyId) {
    ...
    try {
        ...
        const result = await collection.find(query, { projection: { jsonReport: 0, reportOptions: 0, json: 0 } }).toArray();
        db.close();
...
async function setIsSavedTestReport(testReportId, isSaved) {
    ...
    try {
        ...
        result = await collection.findOneAndReplace({ _id: new ObjectId(testReportId) }, updatedReport);
    }
    catch (e) {
    ...
    try {
        ...
        if (repos) {
            for (const repo of repos)
                for (const storyID of repo.stories)
                    await collectionStories.deleteOne({ _id: new ObjectId(storyID) });
...
async function updateOneDriver(id, driver) {
    ...
    try {
        ...
        db = await connectDb();
        const collection = await selectStoriesCollection(db);
        const result = await collection.findOneAndUpdate({ _id: new ObjectId(id) }, { $set: { oneDriver } });
        return result;
    }
    catch (e) {
...
export { updateUser };
export { getUserData };
export { createGitOwnerRepoIfNoneExists };
export { createJiraRepoIfNoneExists };
...
export { selectStoriesCollection };
export { connectDb };
