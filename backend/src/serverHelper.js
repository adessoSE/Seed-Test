...
const key = crypto.scryptSync(
  process.env.JIRA_SECRET,
  process.env.JIRA_SALT,
  32
);
...
function getSteps(steps, stepType) {
  ...
  for (const step of steps) {
    ...
    if (step.values[0] != null && step.values[0] !== "User") {
      data += `${step.pre} '${step.values[0]}' ${step.mid}${getValues(
        step.values
      )}`;
...
function getScenarioContent(scenarios, storyID) {
  ...
  for (const scenario of scenarios) {
    ...
    if (scenario.stepDefinitions.given !== undefined)
      data += `${getSteps(
        scenario.stepDefinitions.given,
        Object.keys(scenario.stepDefinitions)[0]
      )}\n`;
    if (scenario.stepDefinitions.when !== undefined)
      data += `${getSteps(
        scenario.stepDefinitions.when,
        Object.keys(scenario.stepDefinitions)[1]
      )}\n`;
    if (scenario.stepDefinitions.then !== undefined)
      data += `${getSteps(
        scenario.stepDefinitions.then,
        Object.keys(scenario.stepDefinitions)[2]
      )}\n`;
    if (scenario.stepDefinitions.example.length > 0)
...
function writeFile(selectedStory) {
  ...
  fs.writeFile(
    ...
    getFeatureContent(selectedStory),
    (err) => {
      if (err) throw err;
    }
  );
...
function encryptPassword(text) {
  ...
  const cipher = crypto.createCipheriv(cryptoAlgorithm, key, nonce, {
    authTagLength: 16,
  });
...
function decryptPassword(ciphertext, nonce, tag) {
  ...
  try {
    const decipher = crypto.createDecipheriv(cryptoAlgorithm, key, nonce, {
      authTagLength: 16,
    });
...
function analyzeGroupReport(grpName, stories, reportOptions) {
  ...
  try {
    ...
    return pfs.readFile(reportPath, "utf8").then((data) => {
      ...
      console.log(
        `NUMBER OF STORIES IN THE Group-Report: ${cucumberReport.length}`
      );
      try {
        ...
        for (const storyReport of cucumberReport) {
          ...
          try {
            ...
            console.log(
              "iterating through report Json failed in serverHelper/runReport. Setting testStatus of Story to false.",
              error
            );
          }
        }
        // end of for each story
        reportResults.status = testPassed(
          overallPassedSteps,
          overallFailedSteps
        );
        reportResults.groupTestResults = {
          passedSteps: overallPassedSteps,
          failedSteps: overallFailedSteps,
          skippedSteps: overallSkippedSteps,
        };
        ...
        console.log(
          "iterating through report Json failed in analyzeGroupReport." +
            "Setting testStatus of Report to false.",
          error
        );
      }
    });
    ...
    console.log(
      `fs.readFile error for file /features/${grpName}/${grpName}.json`
    );
  }
}
...
function featureResult(featureReport, feature) {
  ...
  const featureStatus = {
    ...
    scenarioStatuses: [],
    featureTestResults: {},
    scenariosTested: { passed: 0, failed: 0 },
  };
  ...
  console.log(
    `NUMBER OF SCENARIOS IN REPORT: ${featureReport.elements.length}`
  );
  for (const scenReport of featureReport.elements) {
    const scenario =
      feature.scenarios[featureReport.elements.indexOf(scenReport)];
  ...
  featureStatus.featureTestResults = {
    passedSteps: featurePassedSteps,
    failedSteps: featureFailedSteps,
    skippedSteps: featureSkippedSteps,
  };
...
function scenarioResult(scenarioReport, scenario) {
  ...
  return {
    ...
    status: scenStatus,
    stepResults: {
      passedSteps: scenarioPassedSteps,
      failedSteps: scenarioFailedSteps,
      skippedSteps: scenarioSkippedSteps,
    },
  };
}
...
function analyzeScenarioReport(stories, reportName, scenarioId, reportOptions) {
  ...
  try {
    ...
    return pfs.readFile(reportPath, "utf8").then((data) => {
      const cucumberReport = JSON.parse(data);
      console.log(
        `NUMBER OF STORIES IN THE REPORT (must be 1): ${cucumberReport.length}`
      );
      try {
        const storyReport = cucumberReport[0];
        console.log(
          `NUMBER OF SCENARIOS IN THE REPORT (must be 1): ${storyReport.elements.length}`
        );
        ...
        const scenario = story.scenarios.find(
          (scen) => scen.scenario_id == scenarioId
        );
        ...
        console.log(
          "iterating through report Json failed in serverHelper/runReport. " +
            "Setting testStatus of Report to false.",
          error
        );
      }
    ...
    if (mode === "group") {
      ...
      for (const story of stories) {
        ...
        if (parameters.isSequential !== undefined && parameters.isSequential) {
          await executeTest(req, res, mode, story).then((values) => {
            callback(values);
          });
        } else {
          executeTest(req, res, mode, story).then((values) => {
            callback(values);
          });
        }
      }
    } else {
      const story = await mongo.getOneStory(
        req.params.issueID,
...
async function resolveReport(reportObj, mode, stories, req, _res, callback) {
  ...
  if ((mode === "feature" || mode === "scenario") && stories.length === 0)
    stories.push(reportObj.story);
  let scenarioId;
  ...
  // analyze Report:
  const reportResults = await analyzeReport(
    req.body.name,
    stories,
    mode,
    reportName,
  ...
  if (mode === "group")
    reportName = `${reportResults.reportName}/${reportResults.reportName}`;
        ...
        mongo
          ...
          .catch((error) => {
            console.log(`Could not UploadReport :  ./features/${reportName}.json
				Rejection: ${error}`);
            res.json({
              htmlFile: `Could not UploadReport :  ./features/${reportName}.json`,
            });
          });
        ...
        for (const story of stories) {
          ...
          if (mode === "group") {
            ...
            comment += renderComment(
              reportResults.groupTestResults.passedSteps,
              reportResults.groupTestResults.failedSteps,
              reportResults.groupTestResults.skippedSteps,
              reportResults.status,
              reportResults.scenariosTested,
              reportResults.reportTime,
              story,
              story.scenarios[0],
              mode,
              reportName.split("/")[0]
            ...
            comment += renderComment(
              reportResults.featureTestResults.passedSteps,
              reportResults.featureTestResults.failedSteps,
              reportResults.featureTestResults.skippedSteps,
              reportResults.status,
              reportResults.scenariosTested,
              reportResults.reportTime,
              story,
...
const nameSchemeChange = async (story) => {
  ...
  if (
    !(await fs.promises
      .stat(
        `./${featuresPath}/${cleanFileName(
          story.title + story._id.toString()
        )}.feature`
      )
      .catch(() => null))
  )
  ...
  if (
    await fs.promises
      .stat(`./${featuresPath}/${cleanFileName(story.title)}.feature`)
      .catch(() => null)
  )
    fs.unlink(
      `./${featuresPath}/${cleanFileName(story.title)}.feature`,
      (err) => console.log("failed to remove file", err)
    );
};
...
async function deleteFeatureFile(storyTitle, storyId) {
  try {
    fs.unlink(
      `features/${cleanFileName(storyTitle + storyId)}.feature`,
      (err) => {
        if (err) throw err;
...
function executeTest(req, _res, mode, story) {
  return new Promise((resolve, _reject) => {
    ...
    const featurePath = `../../features/${cleanFileName(
      story.title + story._id
    )}.feature`;
    const reportName =
      req.user && req.user.github
        ? `${req.user.github.login}_${reportTime}`
        : `reporting_${reportTime}`;
    ...
    const runner = ch.spawn(cucumberCommand, cucumberArgs, {
      cwd: cucumberPath,
    });
...
async function jiraProjects(user) {
  return new Promise((resolve) => {
    if (
      ...
      typeof user.jira === "undefined" ||
      user.jira === null
    )
      resolve([]);
    // eslint-disable-next-line prefer-const
    let { Host, AccountName, Password, Password_Nonce, Password_Tag } =
      user.jira;
      ...
      .then(async (json) => {
        ...
        if (Object.keys(projects).length !== 0) {
          for (const repo of projects) {
            if (
              !jiraReposFromDb.some((entry) => entry.repoName === repo.name)
            ) {
              jiraRepo = await mongo.createJiraRepo(repo.name);
            } else {
              jiraRepo = jiraReposFromDb.find(
                (element) => element.repoName === repo.name
              );
            }
            names.push({
              name: repo.name,
      ...
      for (const repo of data) {
        ...
        if (
          ...
          if (mongoRepo.gitOwner === githubId)
            mongo.updateOwnerInRepo(repo.full_name, ownerId, "github");
        }
...
async function fuseStoryWithDb(story) {
  const result = await mongo.getOneStory(
    parseInt(story.story_id, 10),
    story.storySource
  );
  if (result !== null) {
  ...
  const finalStory = await mongo.upsertEntry(
    story.story_id,
    story,
    story.storySource
  );
...
async function createReport(res, reportName) {
  ...
  fs.writeFileSync(resolvedPath, report.jsonReport, (err) => {
...
async function updateStoryTestStatus(
  storyId,
  storyLastTestStatus,
  scenarioStatuses
) {
...
function renderComment(
  ...
  if (mode === "scenario")
    ...
    comment = `# Test Result ${new Date(
      ...
      scenariosTested.passed
    } :white_check_mark:\nScenarios failed: ${
      scenariosTested.failed
    } :x:\nLink to the official report: [Report](${reportUrl})`;
  return comment;
}
...
async function postCommentGitHub(
  ...
  const auth =
    "Basic " +
    Buffer.from(`${githubName}:${password}`, "binary").toString("base64");
  /** @type {Response} */
...
async function postCommentJira(issueId, comment, host, jiraUser, jiraPassword) {
  ...
  const auth =
    "Basic " +
    Buffer.from(`${jiraUser}:${jiraPassword}`, "binary").toString("base64");
...
function checkValidGithub(userName, repoName) {
  const githubUsernameCheck = new RegExp(
    /^[a-z\d](?:[a-z\d]|-(?=[a-z\d])){0,38}$/i
  ); // https://github.com/shinnn/github-username-regex
  const githubReponameCheck = new RegExp(/^([a-z\d._-]){0,100}$/i);
  return !!(
    githubUsernameCheck.test(userName.toString()) &&
    githubReponameCheck.test(repoName.toString())
  ); // !!
}
