...
function getSteps(steps, stepType) {
    ...
    for (const step of steps) {
        ...
        if ((step.values[0]) != null && (step.values[0]) !== 'User') {
            ...
            if (step.post !== undefined)
                data += ` ${step.post}${step.values[2] !== undefined ? `'${step.values[2]}'` : ''}`;
        }
...
function getExamples(steps) {
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        // eslint-disable-next-line no-continue
        if (steps[i].deactivated || steps[i].values.every((it) => it.trim() === '' || it === 'value'))
            continue;
        data += '\n | ';
...
function writeFile(story) {
    ...
    fs.writeFile(path.join(__dirname, '../features', `${cleanFileName(filename)}.feature`), getFeatureContent(story), (err) => {
        if (err)
            throw err;
    });
}
...
function scenarioPrep(scenarios, driver, globalSettings) {
    ...
    scenarios.forEach((scenario) => {
        ...
        if (!scenario.stepWaitTime)
            scenario.stepWaitTime = 0;
        // eslint-disable-next-line no-param-reassign
        if (!scenario.browser)
            scenario.browser = 'chrome';
        // eslint-disable-next-line no-param-reassign
        if (!scenario.daisyAutoLogout)
            scenario.daisyAutoLogout = false;
        const additionalParams = getSettings(scenario, globalSettings);
        if (scenario.stepDefinitions.example.length <= 0) {
...
function updateLatestTestStatus(uploadedReport, mode) {
    switch (mode) {
        case 'scenario':
            ...
            break;
        case 'feature':
            updateStoryTestStatus(uploadedReport.storyId, uploadedReport.status, uploadedReport.scenarioStatuses);
            break;
        case 'group':
...
async function replaceRefBlocks(scenarios) {
    if (!scenarios.some((scen) => scen.hasRefBlock))
        return scenarios;
    const retScenarios = [];
...
function calcDate(value) {
    ...
    if (day)
        day = parseInt(day[0].match(/@@Day,(\d+)/)[1]);
    let month = value.match(/(@@Month,\d{1,2})/g);
    if (month)
        month = parseInt(month[0].match(/@@Month,(\d+)/)[1] - 1);
    let year = value.match(/(@@Year,\d\d\d\d)/g);
    if (year)
        year = parseInt(year[0].match(/@@Year,(\d+)/)[1]);
    currDate.setFullYear(year == null ? currDate.getFullYear() : year, month == null ? currDate.getMonth() : month, day == null ? currDate.getDate() : day);
...
export { executeTest };
export { updateLatestTestStatus };
export { getReportHistory };
export { cleanFileName };
export { getFeatureContent };
export { getScenarioContent };
export { writeFile };
export { getExamples };
export { getSteps };
export { getBackgroundContent };
export { getBackgroundSteps };
export { getValues };
export { updateFeatureFile };
export { deleteFeatureFile };
export { exportSingleFeatureFile };
export { exportProjectFeatureFiles };
export { applySpecialCommands };
export default {
    executeTest,
