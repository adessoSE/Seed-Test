...
export async function runSanityTest(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
        ...
        const testReq = {
            ...req, // Pass original request context if needed by executeTest
            body: {
                ...group, // Pass group config
                name: featureFileService.cleanFileName(`sanity_${group.name}_${Date.now()}`), // Unique name for report dir
                repositoryId: repoID,
                // Add any other params executeTest expects from req.body
            },
