	...
	try {
		...
		// --- Logic for importing into an EXISTING project ---
			await session.withTransaction(async (currentSession) => {
				await processImport(zip, isRenameMode, repoId, undefined, client, currentSession, userId);
			});
		else {
			...
			await session.withTransaction(async (currentSession) => {
				...
				const newRepoId = (await repositoryService.createRepo(repoData.owner, projectName, currentSession, client)).toHexString();
				...
				await processImport(zip, isRenameMode, newRepoId, projectName, client, currentSession, userId);
			});
		}
		return { success: true };
	} finally {
 ...
 */
async function processImport(zip: AdmZip, isRenameMode: boolean, repoId: string, repoName: string | undefined, client: MongoClient, session: ClientSession, userId?: string) {
	...
	for (const entry of groupEntries) {
		...
		// Map old story IDs to their new ObjectIds from the import mapping
		groupObject.member_stories = groupObject.member_stories
			.map(oldId => oldToNewStoryIdMap.get(oldId.toString())!)
