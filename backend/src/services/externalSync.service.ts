...
import { jiraDecryptPassword, buildAuthString } from "./externalAccount.service";
import { checkValidGithubFormat } from './externalAccount.service';
...
export async function getStoriesFromSource(user: User, query: { [key: string]: string }): Promise<Story[]> {
    ...
    let storiesArray: ObjectId[] = [];
    let repo: any = null; // To store the fetched repo doc
    ...
    if (source === Sources.GITHUB) {
        ...
        const searchUrl = `https://${Host}/rest/api/2/search?jql=project="${projectKey}"+AND+(labels=Seed-Test+OR+issuetype=Test+OR+issuetype="Test Set"+OR+issuetype="Pre-Condition")&startAt=0&maxResults=200`;
