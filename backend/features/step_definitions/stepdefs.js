...
When('I click the button: {string}', async function (button) {
	await driver.getCurrentUrl().then(async (currentUrl) => {
	  ...
	  if ((currentUrl === 'http://localhost:4200/' || currentUrl === 'https://seed-test-frontend.herokuapp.com/') && button.toLowerCase().match(/^run[ _](story|scenario)$/) !== null) throw new Error('Executing Seed-Test inside a scenario is not allowed, to prevent recursion!');
	   else try {
...
Then('So I can see the text {string} in the textbox: {string}', async function (string, label) {
	...
	try {
		await driver.wait(until.elementLocated(By.xpath(`${'//*[@id="'}${label}"]`)), 3 * 1000).then(async (link) => {
			const resp = await link.getText()
				.then(text => text);
			expect(string)
				.to
				.equal(resp, 'Error');
